print("[NHL LINEUP DEBUG] attach_lineup_features called. cols=", list(scored_games.columns))
print("[NHL LINEUP DEBUG] abbr sample:", scored_games.get("home_team_abbr", pd.Series()).head(3).tolist(),
      scored_games.get("away_team_abbr", pd.Series()).head(3).tolist())
print("[NHL LINEUP DEBUG] id sample:", scored_games.get("home_team_id", pd.Series()).head(3).tolist(),
      scored_games.get("away_team_id", pd.Series()).head(3).tolist())

---

print("[NHL LINEUP DEBUG] tricodes=", tricodes)
print("[NHL LINEUP DEBUG] team_ids_bdl(from /teams)=", team_ids_bdl)
...
if has_team_ids:
    ...
    team_ids = raw_ids.dropna().astype(int).drop_duplicates().tolist()
...
[RUN] Using normal script: nhl_quant_main.py
[NHL MAIN] Running (PT) for date=2026-01-07
[SCHED DEBUG] time_col picked: start_time_utc
[SCHED DEBUG] columns: ['game_id', 'start_time_utc', 'game_date', 'date', 'season', 'home_team_id', 'away_team_id', 'home_team_abbr', 'away_team_abbr', 'home_team', 'away_team', 'home_score', 'away_score', 'status', 'postseason', 'period', 'time']
[SCHED DEBUG] time sample: ['2026-01-04T19:00:00.000Z', '2026-01-04T20:00:00.000Z', '2026-01-04T22:00:00.000Z']
Traceback (most recent call last):
[SCHED DEBUG] slate_date counts (top):
  File "/home/runner/work/nhl_bot/nhl_bot/nhl_quant_main.py", line 396, in <module>
    main()
  File "/home/runner/work/nhl_bot/nhl_bot/nhl_quant_main.py", line 307, in main
    raise RuntimeError("[NHL MAIN] scored_games missing home_team/away_team after merge (schedule meta join failed).")
RuntimeError: [NHL MAIN] scored_games missing home_team/away_team after merge (schedule meta join failed).
slate_date
2026-01-10    14
2026-01-08    12
2026-01-06    10
...
[DEBUG][ODDS][ATTACH_GAME_ID] rows=218 null_rate=0.000
[SLATE FIX][NHL] odds_game_ids=5 | today_games(after odds filter)=5
[NHL MAIN] Final slate(PT)=2026-01-07 | games=5 | odds_rows=90
