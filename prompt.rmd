...
def _ensure_col(df, target, candidates):
    if target in df.columns:
        return df
    for c in candidates:
        if c in df.columns:
...
if has_full:
    full_df = full_df.copy()
    # model probs
    full_df = _ensure_col(full_df, "p_xgb",    ["p_xgb", "p_win_xgb", "win_prob_xgb", "p_win"])
    full_df = _ensure_col(full_df, "model_p",  ["model_p", "p_model", "p_final", "p_ensemble", "p_win"])
    # pricing
    full_df = _ensure_col(full_df, "american_odds", ["american_odds", "odds", "moneyline_odds"])
    full_df = _ensure_col(full_df, "implied_p", ["implied_p"])
...
드디어 처음으로 봇은 잘 돌았어. 
이메일 로직한번 봐줄래? nhl 도 nba 랑 bet 들어가는 로직이 일치하는지? 

들어갈만한 벳이 하나도 안들어가는걸로 되어 있네. 

=====
nhl email management.
...
from config import EMAIL_HOST, EMAIL_PORT, EMAIL_USER, EMAIL_PASSWORD, EMAIL_TO, NBA_TOTAL_DAILY_RISK
...
def _kelly_fraction(model_p: float, american_odds: float) -> float:
    ...
    Kelly fraction f* 계산 (bankroll 대비 비율).
      ...
      - model_p: 우리 모델이 보는 승률 (0~1)
      - american_odds: 미국식 오즈 (+150, -120 등)
    ...
    f_star = (b * p - q) / b  # (b*p - (1-p)) / b
    if f_star <= 0.0:
        return 0.0

    # 과도한 배팅 방지용 상한 (예: 50% 이상은 잘못된 입력으로 보고 컷)
    return min(f_star, 0.5)
...
def _apply_kelly_sizing(bet_rows: pd.DataFrame, total_daily_risk: float) -> pd.DataFrame:
    ...
    try:
        ...
        total_daily_risk = 100.0 * n_bets  # fallback
...
def send_nba_daily_report(
    reco_df: pd.DataFrame,
    full_df: pd.DataFrame,
    perf_summary: dict | None = None,
) -> None:
    """
    Send daily NBA report email in ENGLISH.
    """
    has_full = full_df is not None and not full_df.empty
    ...
    AUTO_YES_EDGE_MIN = 1.0      # edge% (percent)
    AUTO_YES_MAX_ABS_ODDS = 250  # -250 ~ +250
    AUTO_YES_ONLY_NON_FLIP = True
    ...
    def _market_flip_mask(df: pd.DataFrame) -> pd.Series:
        # market flip = model thinks one side >50% but market implied <50% (or vice versa)
        mp = _to_num(df.get("model_p"))
        ip = _to_num(df.get("implied_p"))
        return ((mp >= 0.5) & (ip < 0.5)) | ((mp <= 0.5) & (ip > 0.5))
    ...
    def _agreement_ok_with_min(df: pd.DataFrame, p_min: float) -> pd.Series:
        """
        2/3 agreement AND the agreed-two (bet-side) probs are each >= p_min.
        Implementation: take XGB/Margin/Elo probs (already bet-side in this table),
        require at least 2 probs >= 0.5, and the 2nd-highest prob >= p_min.
        """
        xgb = _to_num(df.get("p_xgb"))
        mar = _to_num(df.get("p_margin"))
        elo = _to_num(df.get("p_elo"))
        ...
        # if any row has too many NaNs, fail safe
        valid_cnt = np.sum(~np.isnan(P), axis=1)
        has_2 = valid_cnt >= 2

        # 2/3 agreement on bet-side direction (>=0.5)
        agree_cnt = np.nansum((P >= 0.50).astype(float), axis=1)
        agree_ok = agree_cnt >= 2

        # "agreed 2 models >= p_min"  -> 2nd highest probability >= p_min
        # (if 3 agree, still works; if only 2 agree, their min is the 2nd-highest)
        P2 = np.where(np.isnan(P), -1e9, P)
        second_best = np.sort(P2, axis=1)[:, -2]
        pmin_ok = second_best >= float(p_min)
    ...
    def _eligible_by_edge(df: pd.DataFrame, edge_col: str) -> pd.Series:
        edgep = _edge_pct(df, edge_col)  # percent 단위
        flip = _market_flip_mask(df)
        ...
        # --- 기본 조건: XGB & Model 50% 이상 ---
        base_prob_ok = (xgb >= XGB_MIN) & (mp >= MODEL_MIN)

        # --- odds +200 이상이면 edge 기준 더 빡세게 ---
        is_plus200 = (odds >= 200)
        ...
        # - XGB>=55%, Model>=55%
        # - |odds| <= 250
        # - edge% >= -1  (allow down to -1%)
        # - (optional) only non-flip
        # =========================================================
        auto_yes = (
            (xgb >= 0.55) &
            (mp >= 0.55) &
    ...
    # ==============================
    top_games = None
    edge_col = "edge"  # ✅ 반드시 기본값을 줘서 scope 문제 방지
    ...
    # ==============================
    text_lines: list[str] = []
    text_lines.append("NBA Quant Bot – Daily Report")
    text_lines.append("")
    ...
    if top_games is None or top_games.empty:
        ...
        n_elig = int(len(bet_base))
        # total risk from config.py (already imported)
        try:
            total_daily_risk = float(NBA_TOTAL_DAILY_RISK) if NBA_TOTAL_DAILY_RISK is not None else 0.0
        except Exception:
            total_daily_risk = 0.0
        ...
        if n_elig <= 0:
            ...
            text_lines.append(f"Per-game bettable equity: ${per_game:.2f}")
            text_lines.append("")
            # keep for HTML section if you want
            bet_rows_final = bet_base.copy()
    ...
    html_parts.append("<html>")
    html_parts.append("<body style='font-family:Arial,Helvetica,sans-serif; font-size:14px; line-height:1.4;'>")
    html_parts.append("<h2>NBA Quant Bot – Daily Report</h2>")
    ...
    msg = MIMEMultipart("alternative")
    today_str = datetime.now().strftime("%m-%d-%Y")
    msg["Subject"] = Header(f"NBA Quant Bot – Daily Report, {today_str}", "utf-8")
    msg["From"] = EMAIL_USER
    msg["To"] = EMAIL_TO
