...
slate_date_pt = pt_today + timedelta(days=1)  # ✅ PT 기준 "내일"
print(f"[NBA NEXT] Running (PT) for slate_date={slate_date_pt}")


---

print("[NBA NEXT] raw_games columns:", list(raw_games.columns))
time_col = _pick_datetime_col(raw_games)
if time_col is None:
    raise ValueError("[NBA NEXT] raw_games has no datetime-like column to build past games.")

# 표준화: UTC->naive(normalize)로 맞추고 비교 쉽게
...
if raw_games_past.empty:
    print("[NBA NEXT] No past games before slate_date. Abort.")
    return
...
sched_df = client.fetch_games_by_date_range_df(start_date=start_date, end_date=end_date, per_page=200)
if sched_df is None or sched_df.empty:
    print(f"[NBA NEXT] No scheduled games for {start_date}~{end_date}. Abort.")
    return

tomorrow_games = _filter_games_by_pt_slate_date(sched_df, slate_date_pt, tz_pt)
if tomorrow_games.empty:
    print(f"[NBA NEXT] No games found for PT date={slate_date_pt}. Abort.")
    return
...
# ✅ NBA용 fetch 함수명은 네 코드에 맞춰야 함
# 예: odds_client.fetch_nba_moneyline_odds(...)
odds_df_all = odds_client.fetch_nba_moneyline_odds(regions="us", attach_game_id=True)

odds_df = _filter_odds_by_pt_slate_date(odds_df_all, slate_date_pt, tz_pt)
if odds_df.empty:
    print("[NBA NEXT] Odds empty or not matched to PT slate_date. Abort.")
    return
...
if "game_id" in odds_df.columns:
    ...
    print(f"[SLATE FIX][NBA NEXT] odds_game_ids={len(odds_game_ids)} | schedule_rows(after odds filter)={len(tomorrow_games)}")
    if tomorrow_games.empty:
        print("[SLATE FIX][NBA NEXT] No schedule rows matched odds game_id. Expand schedule date range.")
        return
else:
    print("[NBA NEXT] Odds missing game_id. Abort.")
    return
...
if today_df is None or today_df.empty:
    print("[NBA NEXT] No valid feature rows. Abort.")
    return
...
다음은, 
지금까지 nhl_quant_main_next_day.py 로 작업했는데, 지금까지 작업했던걸, nba_quant_main.py 에도 적용 시켜야 해. 
도와줄래? 

드랍인 코드로. 정확하게 어디서 어딜 수정할지. 
