...
def main():
    ...
    # ✅ source-of-truth = tomorrow_games (schedule)
    ...
    if have:
        meta = tomorrow_games[have].drop_duplicates(subset=["game_id"])
    ...
    if "home_team" not in scored_games.columns or "away_team" not in scored_games.columns:
        raise RuntimeError("[NHL NEXT] scored_games missing home_team/away_team after merge (schedule meta join failed).")
